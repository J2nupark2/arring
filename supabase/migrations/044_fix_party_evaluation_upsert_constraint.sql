-- Use a named unique constraint for evaluator/target upserts so the
-- target_user_id argument cannot conflict with the target_user_id column name.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conrelid = 'public.party_evaluations'::regclass
      and conname = 'party_evaluations_evaluator_target_unique'
  ) then
    alter table public.party_evaluations
      add constraint party_evaluations_evaluator_target_unique
      unique (evaluator_user_id, target_user_id);
  end if;
end;
$$;
